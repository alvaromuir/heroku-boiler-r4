class User < ActiveRecord::Base
  ...
  def self.find_for_twitter_oauth(auth)
    where(auth.slice(:provider, :uid)).first_or_initialize.tap do |user|
      user.provider   = auth.provider
      user.uid        = auth.uid
      user.email      = "unknown_twitter_user_#{auth.uid}@heroku-boiler-r4.com"
      user.password   = Devise.friendly_token[0,20]
  ...
  def self.find_for_windowslive(auth)
    where(auth.slice(:provider, :uid)).first_or_initialize.tap do |user|
      user.provider   = auth.provider
      user.uid        = auth.uid
      user.email      = auth.info.email || "unknown_windowslive_user_#{auth.uid}@heroku-boiler-r4.com"
      user.password   = Devise.friendly_token[0,20]
      user.first_name = auth.info.name.split(' ')[0]
      user.last_name  = auth.info.name.split(' ')[1]
  ...
  def self.find_for_meetup_oauth(auth)
    ...
    where(auth.slice(:provider, :uid)).first_or_initialize.tap do |user|
      ...
      user.email      = "unknown_meetup_user_#{auth.uid}@heroku-boiler-r4.com"
      user.password   = Devise.friendly_token[0,20]
      user.first_name = auth.info.name.split(' ')[0]
      user.last_name  = auth.info.name.split(' ')[1]
      user.avatar     = auth.info.image
